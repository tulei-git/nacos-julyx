...
public class HostReactor implements Closeable {
    ...
    /**
     * NacosNamingService 中调用构造方法创建监听
    ...
    public ServiceInfo getServiceInfo(final String serviceName, final String clusters) {
        ...
        if (null == serviceObj) { // 如果为空，表示本地缓存不存在
            // 如果找不到则创建一个新的然后放入serviceInfoMap，
            // 同时放入updatingMap，执行 updateServiceNow，再从updatingMap移除
        ...
        // 如果本地缓存中存在，则通过scheduleUpdateIfAbsent开启定时任务，
        // 再从serviceInfoMap取出 serviceInfo
        // pull请求更新服务信息
    ...
    public class UpdateTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                if (serviceObj == null) { // 如果为空则立即发起更新
                    updateService(serviceName, clusters);
                    return;
                }

                if (serviceObj.getLastRefTime() <= lastRefTime) { // 判断是否过期
                    updateService(serviceName, clusters);
