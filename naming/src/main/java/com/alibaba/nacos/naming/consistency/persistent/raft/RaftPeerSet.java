...
public class RaftPeerSet extends MemberChangeListener {
    ...
    public RaftPeer decideLeader(RaftPeer candidate) {
        ...
        SortedBag ips = new TreeBag(); // 存储票据数量的集合
        int maxApproveCount = 0; // 最大票据数量
        String maxApprovePeer = null; // 最大票据节点
        for (RaftPeer peer : peers.values()) {
            ...
            if (ips.getCount(peer.voteFor) > maxApproveCount) {
                maxApproveCount = ips.getCount(peer.voteFor); // 同步最大票据
                maxApprovePeer = peer.voteFor; // 同步最大票据节点
            }
        }
        // 判断最大票据是否大于50%
        if (maxApproveCount >= majorityCount()) {
            RaftPeer peer = peers.get(maxApprovePeer);
            peer.state = RaftPeer.State.LEADER; // 最大票据节点状态设置为leader
